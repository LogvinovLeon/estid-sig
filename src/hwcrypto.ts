...
export var hwcrypto = (function hwcrypto() {
  "use strict";
    ...
    this.getCertificate = function(options) {
      ...
      return new Promise(function(resolve, reject) {
        try {
          ...
          var v =
            ver[0] >= 3 && ver[1] >= 13
              ? p.getCertificate(options.filter)
              : p.getCertificate();
      ...
      return new Promise(function(resolve, reject) {
        ...
        if (cid) {
          try {
            ...
            var v =
              ver[0] >= 3 && ver[1] >= 13
                ? p.sign(cid, hash.hex, language, info)
                : p.sign(cid, hash.hex, language);
            resolve({
              hex: v
            });
          } catch (ex) {
  ...
  function _autodetect(force) {
    return new Promise(function(resolve, reject) {
      ...
      if (
        /*navigator.userAgent.indexOf("Chrome") != -1 &&*/ hasExtensionFor(
          digidoc_chrome
        )
      ) {
  ...
  fields.sign = function(cert, hash, options) {
    ...
    if (hash.hex && !hash.value) {
      _debug(
        "DEPRECATED: hash.hex as argument to sign() is deprecated, use hash.value instead"
      );
